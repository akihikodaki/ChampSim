...
void do_stack_pointer_folding(ooo_model_instr& arch_instr)
{
  ...
  bool writes_sp = std::count(std::begin(arch_instr.destination_registers), std::end(arch_instr.destination_registers), champsim::arch.reg_stack_pointer);
  if (writes_sp) {
    ...
    bool reads_other = std::count_if(std::begin(arch_instr.source_registers), std::end(arch_instr.source_registers), [](uint8_t r) {
      return r != champsim::arch.reg_stack_pointer && r != champsim::arch.reg_flags && r != champsim::arch.reg_instruction_pointer;
    });
...
void O3_CPU::do_execution(ooo_model_instr& rob_entry)
{
  ...
  for (auto& sq_entry : SQ) {
    if (sq_entry.instr_id == rob_entry.instr_id) {
      ...
      sq_entry.size = rob_entry.ls_size;
      sq_entry.wdata = rob_entry.source_reg_val[0];
    }
  }
...
bool O3_CPU::do_complete_store(const LSQ_ENTRY& sq_entry)
{
  ...
  if (!L1D_bus.issue_write(data_packet)) {
    return false;
  }

  mem_data[cpu].write(sq_entry.virtual_address, sq_entry.wdata, sq_entry.size);
