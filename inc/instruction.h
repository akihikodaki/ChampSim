...
struct ooo_model_instr : champsim::program_ordered<ooo_model_instr> {
  ...
  void prinf_info()
  {
    ...
    char msg[32];
    fmt::print("op: {}, pc: {}, inst: {}, {:#x}\n", fmt::underlying(op), this->ip, msg);
    ...
    // register
    printf("\nsource_registers     : %02d", this->source_registers[0]);
    ...
    // memory
    fmt::print("\nsource_memory     : {}", this->source_memory[0]);
    for (uint64_t i = 1; i < NUM_INSTR_SOURCES; i++) {
      fmt::print(", {}", this->source_memory[i]);
    }
    fmt::print("\ndestination_memory: {}", this->destination_memory[0]);
    for (uint64_t i = 1; i < NUM_INSTR_DESTINATIONS_SPARC; i++) {
      fmt::print(", {}", this->destination_memory[i]);
    }
...
public:
  ...
  ooo_model_instr(uint8_t cpu, riscv_instr instr) : ooo_model_instr(instr, {cpu, cpu})
  {
    ...
    branch = static_cast<branch_type>(instr.is_branch);
    ...
    if (!cs_disasm(handle, code, size, instr.ip, 1, &insn)) {
      err = cs_errno(handle);
      cs_close(&handle);
    ...
    for (uint8_t i = 0; i < insn->detail->riscv.op_count; i++) {
      ...
      switch (insn->detail->riscv.operands[i].type) {
      case RISCV_OP_IMM:
  ...
  void collect_info(riscv_instr instr)
  {
    ...
    if (is_load) {
      ...
      if (hit_item != pc_info.end()) {
        ...
        auto old_stride_conf = hit_item->second.stride_conf;
