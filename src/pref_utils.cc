...
bool AGQ::update_src(DCT& dct, vector<AGQ_ITEM>::iterator ret_item, uint64_t ret_val)
{
  ...
  for (auto it = dct.buffer.begin(); it != dct.buffer.end() && found_num < ISQ_WRITE_PORT; it++) {
    ...
    if (it->valid && !it->head && it->last_dct_ptr == dct_ptr && need_handle) {
      if (!has_found) { // first item
        ...
        ret_item->dct_ptr = distance(dct.buffer.begin(), it);
        ...
        new_agq_item.dct_ptr = distance(dct.buffer.begin(), it);
...
uint64_t DCT::insert(pt_format_t* pt, AGQ& agq, DCT_ITEM& item)
{
  if (is_full()) {
    // remove_head_chain();
    clear_dct(pt, agq);
...
void DCT::clear_dct(pt_format_t* pt, AGQ& agq)
{
  for (auto it = buffer.begin(); it != buffer.end(); it++) {
    it->valid = false;
  }
...
void AGQ::check_state(const DCT& dct)
{
  bool error = false;
  if (buffer.size() > SIZE) {
    error = true;
  ...
  for (auto it = buffer.begin(); it != buffer.end(); it++) {
    if (!dct.buffer[it->dct_ptr].valid) {
      error = true;
...
void idm_load_return(DCT& dct, AGQ& agq, MEMORY_DATA& mem_data, uint64_t addr)
{
  auto it = agq.search_addr(addr);
  while (it != agq.buffer.end()) {
    auto idm_issue_item = dct.buffer.begin() + it->dct_ptr;
    ...
    uint64_t ret_val = mem_data.read(vaddr, size, !unsign_ext);

    bool has_successor = agq.update_src(dct, it, ret_val);
    if (!has_successor) {
      agq.buffer.erase(it);
    }
