...
uint32_t CACHE::l1d_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
...
uint32_t CACHE::l1d_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
#if PRINT_CACHE_FILL_TRACE == 1
  cout << "\t[l1d_prefetcher_cache_fill]"
       << " cycle: " << current_cycle << hex << ", addr: " << addr << endl;
#endif
