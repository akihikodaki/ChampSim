...
SCENARIO("A prefetch MSHR that gets hit is promoted")
{
  ...
  GIVEN("A cache with a prefetch miss")
  {
    ...
    WHEN("A " + std::string{str} + " is issued")
    {
      ...
      {
        champsim::channel::response_type response{testbed.uut.MSHR.front().reqs.front().id,
                                                  testbed.uut.MSHR.front().address,
                                                  testbed.uut.MSHR.front().v_address,
                                                  testbed.uut.MSHR.front().data_promise->data,
                                                  0,
                                                  testbed.uut.MSHR.front().reqs.front().token};
