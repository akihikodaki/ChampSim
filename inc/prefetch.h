...
typedef enum { PREF_NONE, PREF_STRIDE, PREF_PTC, PREF_IMD, PREF_IMD_END } PREF_TYPE;
...
public:
  ...
  champsim::chrono::clock::time_point time;
#endif
...
public:
  ...
  bool update_src(DCT& dct, vector<AGQ_ITEM>::iterator ret_item, uint64_t ret_val);
  void clear_agq();
#ifdef PREFETCH_DEBUG
  string name;
  uint64_t full_cycles = 0;
  void check_state(const DCT& dct);
  void print_buffer();
#endif
};
...
public:
  ...
  inline bool need_handle() { return formed && (conf == 3) && dense; }
  void print();
};

class DCT
{ // instruction chain table
public:
  ...
  bool is_full();
  uint64_t insert(pt_format_t* pt, AGQ& agq, DCT_ITEM& item);
  void clear_dct(pt_format_t* pt, AGQ& agq);
...
extern "C" {
#endif
IDM_OP encode_imd_op_ld(const DCT& dct, AGQ& agq, bool unsign_ext);
