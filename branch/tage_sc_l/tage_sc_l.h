...
{
private:
  PREDICTOR tage_predictor;

public:
  tage_sc_l(O3_CPU* cpu) : champsim::modules::branch_predictor(cpu) {}
  bool predict_branch(champsim::address ip)
  {
    tage_predictor.predDir = tage_predictor.GetPrediction(ip.to<uint64_t>());
    return tage_predictor.predDir;
  }
  void last_branch_result(champsim::address ip, champsim::address branch_target, bool taken, uint8_t branch_type)
  {
    tage_predictor.UpdatePredictor(ip.to<uint64_t>(), branch_type, taken, branch_target.to<uint64_t>());
  }
};
